...
function isCityOnlyPatch (input) {
    if (!input || typeof input !== 'object') { return false; }
    const hasCity = input.city !== undefined && input.city !== null;
    const hasLat = numberValue(input.lat, -90, 90) !== null;
    const hasLng = numberValue(input.lng, -180, 180) !== null;
    return hasCity && !hasLat && !hasLng;
}

function locationPayload (home, timezone) {
    const tz = timezone || {};
    return {
        city: home.city || null,
...
function current () {
    return loadLocationNodes().then((loaded) => {
        ...
        return {
            available: true,
            location: locationPayload(home, timezone)
        };
    });
}
...
function applyCityOnly (input) {
    const city = stringValue(input.city, 120);
    if (!city) {
        throw fail('A town or city name is required', 400);
    }

    return loadLocationNodes().then((loaded) => {
        const home = loaded.home.data || {};
        const timezone = loaded.timezone.data || {};
        const lat = numberValue(home.lat, -90, 90);
        const lng = numberValue(home.lng, -180, 180);
        if (lat === null || lng === null) {
            throw fail(
                'Find my location first so coordinates and timezone are set.',
                400
            );
        }

        const nextHome = Object.assign({}, home, { city: city });
        loaded.home.data = nextHome;
        return loaded.home.save().then(() => ({
            ok: true,
            location: locationPayload(nextHome, timezone),
            refreshedInMemory: refreshInMemory(loaded.jibo, nextHome, timezone),
...
function apply (input) {
    if (isCityOnlyPatch(input)) {
        return applyCityOnly(input);
    }
    ...
    return loadLocationNodes().then((loaded) => {
        ...
        return loaded.home.save().then(() => loaded.timezone.save()).then(() => ({
            ok: true,
            location: locationPayload(next.home, next.timezone),
