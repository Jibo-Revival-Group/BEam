...
Buttons live in JSON view configs under `@be/main-menu/resources/views/`.
...
- **`iconSrc`** — e.g. `resources/icons/bad-apple.png` (under `@be/main-menu`).
...
Implemented in `@be/main-menu/index.js`:
...
```
@be/main-menu/resources/icons/<name>.png
```

Reference as `"iconSrc": "resources/icons/<name>.png"` in the verbal JSON.

After changing main-menu resources, rebuild/sync `@be/main-menu` if your workflow bundles it (the verbal JSON is typically loaded at runtime).
...
3. Icon added under `@be/main-menu/resources/icons/`.
